...
// grailum includes.
#include <..\src\script\grailumconsensus.h>
...
int main()
{
    std::cout << "grailumconsensus version: " << grailumconsensus_version() << std::endl;
    ...
    auto op0Result = grailumconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, grailumconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);

...
static fs::path LexicallyRelative(const fs::path& path, const fs::path& base)
{
     ...
     return path.string().substr(base.string().size() + 1); // Account for directory separator
}
...
std::vector<fs::path> ListWalletDir()
{
    ...
    for (auto it = fs::recursive_directory_iterator(wallet_dir); it != fs::recursive_directory_iterator(); ++it) {
        if (it->status().type() == fs::directory_file && IsBerkeleyBtree(it->path() / "wallet.dat")) {
            ...
            paths.emplace_back(LexicallyRelative(it->path(), wallet_dir));
            ...
            if (it->path().filename() == "wallet.dat") {
                ...
                // Found top-level btree file not called wallet.dat. Current grailum
